  ...
  return (
    <div className="min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 py-8 max-w-5xl">
        ...
        <div className={`grid grid-cols-1 gap-8 ${batch ? 'lg:grid-cols-3' : ''}`}>
          {/* Main content */}
          <div className={`space-y-8 ${batch ? 'lg:col-span-2' : ''}`}>
            ...
            <div>
              <h2 className="text-lg font-semibold text-foreground mb-4">Technologies Covered</h2>
              <div className="flex flex-wrap gap-2">
                {course.techStack?.map((tech) => (
                  <span key={tech} className="px-3 py-1.5 rounded-lg border border-indigo-500/20 bg-indigo-500/10 text-sm text-indigo-500 dark:text-indigo-400">
                    {tech}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {course.syllabus?.map((week, i) => (
                  <div key={i} className="rounded-xl border border-border bg-muted/20 overflow-hidden">
                    ...
                    >
                      <span className="font-medium text-foreground text-sm">Week {week.week}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-muted-foreground">{week.topics?.length} topics</span>
                        {openWeek === i ? <ChevronUp size={14} className="text-muted-foreground" /> : <ChevronDown size={14} className="text-muted-foreground" />}
                      </div>
                    </button>
                    {openWeek === i && (
          ...
          {/* Sidebar card */}
          {batch && <div className="lg:col-span-1">
            <Card className="sticky top-20 border-indigo-500/20">
              <CardHeader className="pb-4">
                <CardTitle className="text-2xl font-bold text-foreground">
                  {batch?.name && (
                    <p className="text-lg text-muted-foreground mt-1">{batch.name}</p>
                )}
      ...
      {showPayment && (
        <>
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setShowPayment(false)} />
          <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 bg-white/90 dark:bg-black/60 -translate-y-1/2 rounded-2xl border border-border bg-background p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-foreground">Complete Enrollment</h2>
              <button onClick={() => setShowPayment(false)} className="text-muted-foreground hover:text-foreground p-1 rounded-lg hover:bg-muted/50">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-bold text-lg">₹{displayFee?.toLocaleString('en-IN')}</span>
              </div>
            </div>
