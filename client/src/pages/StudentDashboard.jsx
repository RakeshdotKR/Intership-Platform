...
import {
  BookOpen, Clock, IndianRupee, CalendarDays, ArrowRight,
  GraduationCap, X, Users, Layers,
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        <div className="relative w-full max-w-lg max-h-[88vh] flex flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-black/10 overflow-hidden dark:border-white/10 dark:bg-[#0d0d1a] dark:shadow-black/60">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0 dark:border-white/8">
            <div className="flex items-center gap-2 min-w-0">
              <h2 className="text-base font-semibold text-slate-900 truncate dark:text-white">{batch?.name}</h2>
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {batch?.courses?.[0]?.image ? (
              ...
              <div className="w-full h-40 bg-linear-to-br from-indigo-50 to-violet-50 flex items-center justify-center dark:from-indigo-500/10 dark:to-violet-500/10">
            ...
            <div className="flex flex-col gap-5 p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  },
                  {
                    icon: <Users size={13} />,
                    label: 'Batch Size',
                    value: batch?.maxStudents ? `${batch.maxStudents} seats` : '—',
                  },
                ].map(({ icon, label, value }) => (
                  <div key={label} className="flex flex-col gap-1 px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/4 dark:border-white/6">
                    <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-white/40">{icon}{label}</span>
              ...
              {batch?.description && (
                <>
                  <Separator />
                  <div>
                    <p className="text-xs uppercase tracking-widest text-slate-400 mb-2 dark:text-white/30">About this batch</p>
                    <p className="text-sm text-slate-600 leading-relaxed dark:text-white/60">{batch.description}</p>
                  </div>
                </>
              )}
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {batch?.courses?.length ? batch.courses.map((course, idx) => (
                    <div
                      key={course.id}
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm text-slate-900 font-medium truncate dark:text-white">{course.title}</span>
                        {course.duration && (
                    ...
                    <p className="text-sm text-slate-400 dark:text-white/30">No courses listed.</p>
                  )}
                </div>
              </div>
              ...
              {enrollment?.enrolledAt && (
                <>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 dark:text-white/30">
                    ...
                    <span className="text-slate-600 dark:text-white/50">{fmt(enrollment.enrolledAt)}</span>
                  </div>
                </>
              )}
            </div>
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const handleKey = (e) => { if (e.key === 'Escape') setSelectedEnrollment(null); };
        ...
        <div className="relative container mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{user?.name?.split(' ')[0]}'s Dashboard</h1>
              <p className="text-sm text-slate-500 mt-1 dark:text-white/40">{user?.college} · {user?.branch}</p>
            </div>
            <Link to="/courses">
          ...
          {!loading && enrollments.length > 0 && (
            <div className="flex flex-wrap gap-4 mt-6">
              {[
                ...
                <div key={label} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 dark:bg-white/5 dark:border-white/8">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-10">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 border border-dashed border-slate-300 rounded-2xl dark:border-white/10">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2 dark:text-white">No courses yet</h3>
          ...
          <>
            <h2 className="text-base font-semibold text-slate-900 mb-5 dark:text-white">My Enrolled Programs</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                return (
                  <Card key={enrollment.id} className="flex flex-col hover:border-slate-300 dark:hover:border-white/10 transition-all overflow-hidden">
                    {firstCourse?.image ? (
                      ...
                      <div className="h-36 bg-linear-to-br from-indigo-50 to-violet-50 flex items-center justify-center dark:from-indigo-500/10 dark:to-violet-500/10">
                        <BookOpen size={28} className="text-indigo-300 dark:text-white/15" />
                      </div>
                    )}
                    <CardContent className="p-5 flex flex-col flex-1">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h3 className="font-semibold text-slate-900 text-sm leading-snug dark:text-white">{batch?.name}</h3>
                        <Badge variant={statusCfg.variant} className="shrink-0">{statusCfg.label}</Badge>
                      </div>
                      <div className="flex flex-wrap gap-1 mb-3">
                        {batch?.courses?.map(c => (
                          <span key={c.id} className="text-xs text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded dark:text-white/40 dark:bg-white/5">
                            {c.title}
                          </span>
                        ))}
                      </div>
